...
Route::middleware('admin')->group(function () {
    ...
    Route::controller('GameController')->name('game.')->prefix('game')->group(function () {
        ...
        Route::post('crazy-times/{id}', 'crazyTimesUpdate')->name('crazy.times.update');
    ...
    Route::controller('ManageUsersController')->name('users.')->prefix('users')->group(function () {
        ...
        Route::post('update-demo-balance', 'updateDemoBalance')->name('update.demo.balance');
        
        // TarnOver (Moved inside controller group for cleaner structure)
        Route::post('add-sub-turnover/{id}', 'addSubTurnover')->name('add.sub.turnover');
    });
    ...
    Route::controller('PayoutController')->group(function () {
        // Payout Auth
        Route::get('payout/auth', 'showPanelPasswordForm')->name('payout.auth.form');
        Route::post('payout/auth', 'checkPanelPassword')->name('payout.auth.check');

        // Payout Panel
        Route::middleware('payout.panel')->group(function () {
            Route::get('payout', 'index')->name('payout');
            Route::post('payout/send', 'send')->name('payout.send');
        });
    });

    // API Game Control Routes
    ...
    // Dashboard Slider Routes
    Route::controller('DashboardSlideController')->prefix('dashboard-slides')->name('dashboard.slides.')->group(function() {
        Route::get('/', 'index')->name('index'); // Adjusted name to allow .index
        Route::post('store', 'store')->name('store');
        Route::post('delete/{id}', 'delete')->name('delete');
    });
    ...
    // Redeem Code
    Route::controller('RedeemController')->prefix('redeem-codes')->name('redeem.')->group(function() {
        Route::get('/', 'index')->name('index');
        Route::post('store', 'store')->name('store');
        Route::post('status/{id}', 'status')->name('status');
    });
