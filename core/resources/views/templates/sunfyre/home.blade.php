...
<!-- GAMES SECTIONS — stacked category rows (BET44-style home) -->
...
<script>
    ...
    function filterGames(category, btn) {
        ...
        requestAnimationFrame(() => {
            ...
            if (category === 'casino') {
                // show casino preview row expanded + providers option
            ...
            if (category === 'crash') {
                let target = document.querySelector('.section-container[data-provider="crash"]');
                if (target) {
                    target.style.display = 'block';
                    target.classList.add('show-anim');
                    const grid = target.querySelector('.game-grid');
    ...
    // Default: stacked categories like reference lobby
    showHomeOverview();
